"""
Render just the title card for quick visual testing.

Usage: python tools/full_reel/test_titlecard.py [--album .tmp/album_data.json] [--duration 5]

Output: .tmp/output/titlecard_test.mp4
"""
...
def main():
    parser = argparse.ArgumentParser(description="Test title card render")
    parser.add_argument("--album", default=".tmp/album_data.json")
    parser.add_argument("--broll-dir", default=".tmp/broll")
    parser.add_argument("--output", default=".tmp/output/titlecard_test.mp4")
    ...
    print(f"Building title card for: {album_data['album']} by {album_data['artist']}")

    start = time.time()
    title_card = build_title_card(album_data, cover_path, broll_manifest, args.broll_dir, duration=args.duration)
